function f=radarplot(R,Lable,LineColor,FillColor,LineStyle,LevelNum,FontSize,Color)
    ...
    if LevelNum>0
        ...
        for i=1:LevelNum
%                     text(AxisR(i)*sin(pi/n-0.3),AxisR(i)*cos(pi/n-0.3),num2str(AxisR(i),2), ...
%                             'FontSize',     10, ...
%                             'FontWeight',   obj.Value_FontWeight)
        end
    ...
    %%
    F = fill(AxisX(end,:),AxisY(end,:),[1,1,1],'LineStyle','none');
        ...
        if length(Lable)>=n
            ...
            for i=1:n
                if ~sum(strcmpi({'' },Lable(i)))
                     h = text(LableX(i), LableY(i),cell2mat(Lable(i)),   'FontSize',             FontSize, ...
